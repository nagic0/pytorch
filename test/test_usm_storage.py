...
class TestUsmStorage(TestCase):
    ...
    def test_usm_share_cpu_to_cpu(self):
        """Test usm_share_ from CPU to CPU (should return same storage)."""
        # Create CPU storage
        cpu_storage = torch.UntypedStorage(1024)
        for i in range(1024):
            cpu_storage[i] = i % 256
        
        # Share to CPU (should be no-op)
        shared_storage = cpu_storage.usm_share_(torch.device('cpu'))
        
        # Should return the same storage reference
        self.assertEqual(cpu_storage.data_ptr(), shared_storage.data_ptr())
        self.assertEqual(cpu_storage.nbytes(), shared_storage.nbytes())
