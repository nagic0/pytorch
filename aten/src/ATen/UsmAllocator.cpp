...
// Attempt to increase the RLIMIT_MEMLOCK (soft limit) to the hard limit.
// This allows mlock to succeed for larger buffers without sudo, provided
// the hard limit permits it.
void try_increase_memlock_limit() {
  struct rlimit rl{};
...
#endif
{
  ...
  // 2. Lock Memory (mlock)
  // --------------------------------------------------------
  // Locking prevents the OS from swapping this memory to disk.
  // This is critical for performance and for sharing memory with GPUs (Unified Memory).
  try_increase_memlock_limit();
  if (mlock(base_ptr_, size_) != 0) {
    // We treat mlock failure as a warning (Soft Fail). 
    // The program can continue, though performance might degrade.
    TORCH_WARN("USM: mlock failed (size=", size_, "). Error: ", strerror(errno), 
               ". Suggestion: Check 'ulimit -l' or system memory limits.");
    // Touch by memset to potentially reduce page faults later
